...
/// 特性: 支持对每个接口设置独立判断规则
public protocol MoyaResultValidateable {

    /// 当 HTTP Status Code == 'success'，判断服务端返回的数据是否符合成功约定
    /// 默认 true，全都符合，当 false 时，返回结果为 Result<Moya.Response, MoyaError>.failure(.underlying(response))
